...
int main(int argc, char *argv[]) {
    ...
    for (int epsilonSize : epsilonSizes) {
        ...
        for (int run = 0; run < warmupRuns + measuredRuns; run++) {
            ...
            fs::path filePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_GridRangeQueryWeighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx");
            error_code ec;
            ...
            fs::path binFilePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_GridRangeQueryWeighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx.bin");
            error_code binec;

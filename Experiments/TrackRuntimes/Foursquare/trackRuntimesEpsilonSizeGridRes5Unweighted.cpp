...
int main(int argc, char *argv[]) {
    ...
    for (int epsilonSize : epsilonSizes) {
        ...
        for (int run = 0; run < warmupRuns + measuredRuns; run++) {
            ...
            fs::path filePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_GridRangeQueryUnweighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx");
            error_code ec;
            ...
            fs::path binFilePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_GridRangeQueryUnweighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx.bin");
            error_code binec;
            ...
            SparseMatrix<unsigned long long, RowMajor> sparseMatrix = loadGridCommutingMatrix<unsigned long long>(allDatasetPath.string(),
                                                                                                                  "Foursquare",
                                                                                                                  "checked_in_at",
                                                                                                                  "venue",
                                                                                                                  "venuesWGS84.csv",

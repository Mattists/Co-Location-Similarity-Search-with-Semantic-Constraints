...
int main(int argc, char *argv[]) {
    ...
    CreateRTreeIfMissing(allDatasetPath, "Foursquare", "venuesWGS84.csv", ',');
...

    {
        ...
        fs::path filePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon) + ".mtx");
        error_code ec;
        ...
        fs::path binFilePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon) + ".mtx.bin");
        error_code binec;
    ...
    vector<long double> deltas2 = loadDeltas<long double>("checked_in_at#venue_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon),
                                                          "Foursquare",
                                                          allDatasetPath);
    ...
    {
        // remove matrix before generating new one to prevent simply loading it from disk
        fs::path filePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon) + ".mtx");
        error_code ec;
        if (fs::exists(filePath)) {
            fs::remove(filePath, ec);
            if (ec) {
        ...
        // also remove bin matrix before generating new one to prevent simply loading it from disk
        fs::path binFilePath = allDatasetPath / "Foursquare" / "matrices" / ("checked_in_at#venue_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon) + ".mtx.bin");
        error_code binec;
        if (fs::exists(binFilePath)) {
            fs::remove(binFilePath, binec);
            if (binec) {
    ...
    vector<unsigned long long> deltas3 = loadDeltas("checked_in_at#venue_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon),
                                                    "Foursquare",
                                                    allDatasetPath);

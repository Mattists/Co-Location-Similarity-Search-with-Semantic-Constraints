...
This directory contains a modified version of [RTree](https://github.com/nushoin/RTree), retrieved in 2024.

The original RTree implementation is distributed under its Public Domain/MIT licensing terms. The original license text is included in the `LICENSE` file in this directory.

Modifications Copyright (c) 2026 Mattis thor Straten.

The modifications include changes to `RTree.h`, which are marked by comments in the source code. In addition, `CMakeLists.txt` were adapted to integrate RTree with the build and testing setup of the surrounding repository and this `README` was adapted.

## Description

A C++ templated version of [this](http://www.superliminal.com/sources/sources.htm) RTree algorithm.
...
When built as part of this repository, enable the RTree tests from the repository root.

```bash
cmake -DBUILD_TESTING=ON -DSPATIALMETAPATH_BUILD_RTREE_TESTS=ON -S . -B build
cmake --build build
cd build
ctest
```

...
int main(int argc, char *argv[]) {
    ...
    CreateRTreeIfMissing(allDatasetPath.string(), "YFCC", "photosWGS84.csv", ',');
    ...
    for (int epsilonSize : epsilonSizes) {
        ...
        for (int run = 0; run < warmupRuns + measuredRuns; run++) {
            ...
            fs::path filePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilonSize) + ".mtx");
            error_code ec;
            ...
            fs::path binFilePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilonSize) + ".mtx.bin");
            error_code binec;
            ...
            SparseMatrix<long double, RowMajor> sparseMatrix = loadEdgeDecompositionCommutingMatrix<long double>(allDatasetPath.string(),
                                                                                                                    "YFCC",
                                                                                                                    "posted",
                                                                                                                    "photo",
                                                                                                                    "photosWGS84.csv",

...
int main(int argc, char *argv[]) {
    ...
    constexpr int gridResolution = 7;
    ...
    for (int epsilonSize : epsilonSizes) {
        ...
        for (int run = 0; run < warmupRuns + measuredRuns; run++) {
            ...
            fs::path filePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_GridRangeQueryUnweighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx");
            error_code ec;
            ...
            fs::path binFilePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_GridRangeQueryUnweighted" + std::to_string(epsilonSize) + "_" + std::to_string(gridResolution) + ".mtx.bin");
            error_code binec;
            ...
            auto t1 = high_resolution_clock::now(); // We measure end-to-end preprocessing time including grid-based matrix construction and disk I/O.

            SparseMatrix<unsigned long long, RowMajor> sparseMatrix = loadGridCommutingMatrix<unsigned long long>(allDatasetPath, 
                                                                                                                  "GeoDBLP", 
            ...
            volatile auto nnz = sparseMatrix.nonZeros(); // prevent optimization
        ...
        // Compute averaged runtimes and standard deviation

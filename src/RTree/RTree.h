...
public:
  ...
  /// Note: Added by Mattis thor Straten.
  /// Find all objects within a specified range given an object
  /// \param a_min Min of given object rect
  /// \param a_max Max of given object rect
  /// \param epsilonRange The maximum distance for an object to be in the range, i.e. the epsilon range
  /// \param a_searchResult Search result array.  Caller should set grow size. Function will reset, not append to array.
  /// \return Returns the found entries together with their actual distance to the query object. Results are not sorted.
  std::vector<std::pair<double, DATATYPE>> RangeQuery(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const int epsilonRange, std::function<bool (const DATATYPE&)> callback) const;
...
protected:
  ...
  double DegreesToRadians(double degrees) const;  /// Note: Added by Mattis thor Straten.
  double RadiansToDegrees(double radians) const;  /// Note: Added by Mattis thor Straten.
  double HaversineDistance(Rect* a_rectA, Rect* a_rectB) const;  /// Note: Added by Mattis thor Straten.
  Rect HaversineSearchRect(Rect* a_rect, int epsilonRange) const;  /// Note: Added by Mattis thor Straten.
  void ReInsert(Node* a_node, ListNode** a_listNode);
  bool Search(Node* a_node, Rect* a_rect, int& a_foundCount, std::function<bool (const DATATYPE&)> callback) const;
  /// Note: Added by Mattis thor Straten.
  bool RangeQuery(Node* a_node, Rect* a_queryRect, Rect* a_searchRect, const int epsilonRange, std::vector<std::pair<double, DATATYPE>>& neighbors, std::function<bool (const DATATYPE&)> callback) const;
...
RTREE_TEMPLATE
std::vector<std::pair<double, DATATYPE>> RTREE_QUAL::RangeQuery(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], const int epsilonRange, std::function<bool (const DATATYPE&)> callback) const
{
#ifdef _DEBUG
  for(int index=0; index<NUMDIMS; ++index)
  {
  ...
  std::vector<std::pair<double, DATATYPE>> neighbors;
  Rect searchRect = HaversineSearchRect(&rect, epsilonRange);
  RangeQuery(m_root, &rect, &searchRect, epsilonRange, neighbors, callback);
...
RTREE_TEMPLATE
double RTREE_QUAL::DegreesToRadians(double degrees) const
{
  return degrees * 3.14159265358979323846 / 180.0;
}

RTREE_TEMPLATE
double RTREE_QUAL::RadiansToDegrees(double radians) const
{
  return radians * 180.0 / 3.14159265358979323846;
}

RTREE_TEMPLATE
double RTREE_QUAL::HaversineDistance(Rect* a_rectA, Rect* a_rectB) const
{
  RTREE_ASSERT(a_rectA && a_rectB);

  if (NUMDIMS != 2)
  {
    throw std::invalid_argument("Haversine distance requires exactly two dimensions: longitude and latitude.");
  }

  constexpr double earthRadiusMeters = 6371008.8;

  const double lon1 = DegreesToRadians(static_cast<double>(a_rectA->m_min[0]));
  const double lat1 = DegreesToRadians(static_cast<double>(a_rectA->m_min[1]));
  const double lon2 = DegreesToRadians(static_cast<double>(a_rectB->m_min[0]));
  const double lat2 = DegreesToRadians(static_cast<double>(a_rectB->m_min[1]));
  ...
  const double c = 2.0 * std::atan2(std::sqrt(clampedA), std::sqrt(1.0 - clampedA));

  return earthRadiusMeters * c;
}

RTREE_TEMPLATE
typename RTREE_QUAL::Rect RTREE_QUAL::HaversineSearchRect(Rect* a_rect, int epsilonRange) const
{
  RTREE_ASSERT(a_rect);

  if (NUMDIMS != 2)
  {
    throw std::invalid_argument("Haversine range queries require exactly two dimensions: longitude and latitude.");
  }
  if (epsilonRange < 0)
  {
    throw std::invalid_argument("The epsilon range for a Haversine range query must be non-negative.");
  }

  constexpr double earthRadiusMeters = 6371008.8;
  const double longitude = static_cast<double>(a_rect->m_min[0]);
  const double latitude = static_cast<double>(a_rect->m_min[1]);
  const double angularDistance = static_cast<double>(epsilonRange) / earthRadiusMeters;
  const double latitudeDelta = RadiansToDegrees(angularDistance);
  const double cosLatitude = std::cos(DegreesToRadians(latitude));

  double longitudeDelta = 180.0;
  if (std::abs(cosLatitude) > 1e-12)
  {
    longitudeDelta = std::min(180.0, RadiansToDegrees(angularDistance / std::abs(cosLatitude)));
  }

  Rect searchRect;

  if (longitude - longitudeDelta < -180.0 || longitude + longitudeDelta > 180.0)
  {
    searchRect.m_min[0] = static_cast<ELEMTYPE>(-180.0);
    searchRect.m_max[0] = static_cast<ELEMTYPE>(180.0);
  }
  else
  {
    searchRect.m_min[0] = static_cast<ELEMTYPE>(longitude - longitudeDelta);
    searchRect.m_max[0] = static_cast<ELEMTYPE>(longitude + longitudeDelta);
  }

  searchRect.m_min[1] = static_cast<ELEMTYPE>(std::max(-90.0, latitude - latitudeDelta));
  searchRect.m_max[1] = static_cast<ELEMTYPE>(std::min(90.0, latitude + latitudeDelta));
...
// Perform a Haversine range query for longitude/latitude points. The search rectangle is only
// a coarse degree-based candidate filter; leaf distances are checked exactly in meters.
RTREE_TEMPLATE
bool RTREE_QUAL::RangeQuery(Node* a_node, Rect* a_queryRect, Rect* a_searchRect, const int epsilonRange, std::vector<std::pair<double, DATATYPE>>& neighbors, std::function<bool (const DATATYPE&)> callback) const
{
  RTREE_ASSERT(a_node);
  RTREE_ASSERT(a_node->m_level >= 0);
  RTREE_ASSERT(a_queryRect);
  RTREE_ASSERT(a_searchRect);

  if(a_node->IsInternalNode())
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_searchRect, &a_node->m_branch[index].m_rect))
      {
        if(!RangeQuery(a_node->m_branch[index].m_child, a_queryRect, a_searchRect, epsilonRange, neighbors, callback))
        {
          return false;
        }
      }
  ...
  else
  {
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_searchRect, &a_node->m_branch[index].m_rect))
      {
        double distance = HaversineDistance(a_queryRect, &a_node->m_branch[index].m_rect);

...
int main(int argc, char *argv[]) {
    ...
    CreateRTreeIfMissing(allDatasetPath, "GeoDBLP", "affiliationsWGS84.csv", ',');
...

    {
        ...
        fs::path filePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon) + ".mtx");
        error_code ec;
        ...
        fs::path binFilePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon) + ".mtx.bin");
        error_code binec;
    ...
    SparseMatrix<long double, RowMajor> sparseMatrix2 = loadEdgeDecompositionCommutingMatrix<long double>(allDatasetPath,
                                                                                               "GeoDBLP",
    ...
    vector<long double> deltas2 = loadDeltas<long double>("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryWeighted" + std::to_string(epsilon),
                                                          "GeoDBLP",
                                                          allDatasetPath);
    ...
    cout << "\nEDGE_DECOMPOSITION-DISTANCE: Objects most similar to object " << queryObject << " using weighted=true:" << endl;
    ...
    {
        // remove matrix before generating new one to prevent simply loading it from disk
        fs::path filePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon) + ".mtx");
        error_code ec;
        if (fs::exists(filePath)) {
            fs::remove(filePath, ec);
            if (ec) {
        ...
        // also remove bin matrix before generating new one to prevent simply loading it from disk
        fs::path binFilePath = allDatasetPath / "GeoDBLP" / "matrices" / ("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon) + ".mtx.bin");
        error_code binec;
        if (fs::exists(binFilePath)) {
            fs::remove(binFilePath, binec);
            if (binec) {
    ...
    SparseMatrix<unsigned long long, RowMajor> sparseMatrix3 = loadEdgeDecompositionCommutingMatrix<unsigned long long>(allDatasetPath,
                                                                                                             "GeoDBLP",
                                                                                                             "published_in_conference^-1#wrote^-1#affiliated_with",
                                                                                                             "affiliation",
                                                                                                             "affiliationsWGS84.csv",
    ...
    vector<unsigned long long> deltas3 = loadDeltas("published_in_conference^-1#wrote^-1#affiliated_with#affiliation_EdgeDecompositionRangeQueryUnweighted" + std::to_string(epsilon),
                                                    "GeoDBLP",
                                                    allDatasetPath);
    ...
    cout << "\nEDGE_DECOMPOSITION-DISTANCE: Objects most similar to object " << queryObject << " using weighted=false:" << endl;

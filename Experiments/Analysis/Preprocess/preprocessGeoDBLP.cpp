...
int main(int argc, char *argv[]) {
    ...
    if (argc != 2) {
        cout << "Usage: ./preprocessGeoDBLP<path_to_all_datasets>" << endl;
        return 1;
    }

    map<string, vector<string>> metaPathsPerDataset = {
            {"GeoDBLP",       {"published_in_conference^-1#wrote^-1#affiliated_with",
                               "published_in_conference^-1#wrote^-1#affiliated_with#located_in"}}
    };
    ...
    for (auto &[dataset, metaPaths]: metaPathsPerDataset) {
        if (dataset == "GeoDBLP") {
            generatePartialCommutingMatrix<unsigned long long>(dataset, allDatasetPath, metaPaths, true, true, true);
        }
    }
}

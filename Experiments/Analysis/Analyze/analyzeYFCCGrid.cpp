...
int main(int argc, char *argv[]) {
...

    {
        ...
        fs::path filePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_GridRangeQueryWeighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution) + ".mtx");
        error_code ec;
        ...
        fs::path binFilePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_GridRangeQueryWeighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution) + ".mtx.bin");
        error_code binec;
    ...
    vector<long double> deltas2 = loadDeltas<long double>("posted#photo_GridRangeQueryWeighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution),
                                                          "YFCC",
                                                          allDatasetPath);
    ...
    {
        // remove matrix before generating new one to prevent simply loading it from disk
        fs::path filePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_GridRangeQueryUnweighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution) + ".mtx");
        error_code ec;
        if (fs::exists(filePath)) {
            fs::remove(filePath, ec);
            if (ec) {
        ...
        // also remove bin matrix before generating new one to prevent simply loading it from disk
        fs::path binFilePath = allDatasetPath / "YFCC" / "matrices" / ("posted#photo_GridRangeQueryUnweighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution) + ".mtx.bin");
        error_code binec;
        if (fs::exists(binFilePath)) {
            fs::remove(binFilePath, binec);
            if (binec) {
    ...
    vector<unsigned long long> deltas3 = loadDeltas("posted#photo_GridRangeQueryUnweighted" + std::to_string(epsilon) + "_" + std::to_string(gridResolution),
                                                    "YFCC",
                                                    allDatasetPath);
